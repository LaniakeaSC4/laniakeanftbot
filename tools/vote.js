...
//when a vote button is pressed
async function sendVoteModal(interaction) {
...
} module.exports.sendVoteModal = sendVoteModal
  ...
  if (updown === 'down') {
	...
	for (var i = 0; i < supportedcollections.length; i++) {//loop supported collections recieved from SQL
		if (supportedcollections[i].collectionkey === meslug) {//if collection entered by user is found in our supported collections
			found = true
			w.log.info('validated collection. We can register a vote for this one')
			//add votes row
...
async function addVote(server_id, user_id, votetype, votemeslug) {
  return new Promise((resolve, reject) => {
    ...
    var querystring = "INSERT INTO votes(server_id, user_id, votetype, votemeslug) VALUES ($1,$2,$3,$4)"
